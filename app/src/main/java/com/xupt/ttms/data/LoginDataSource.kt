...
class LoginDataSource {
    ...
    private suspend fun requestCode (phone: String) :Boolean {
        return try {
            val result = loginService.getCode(RequestBody.create(MediaType.parse("application/json"),gson.toJson(CodeRequest(phone))))
            result.data
        } catch (e:Exception) {
            false
        }
    ...
    private suspend fun requestLogin(phone:String, code:String) :Boolean {
        return try{
            val result = loginService.judgeCode(RequestBody.create(MediaType.parse("application/json"), gson.toJson(LoginRequest(phone, code))))
            result.data
        } catch (e: Exception) {
            false
        }
